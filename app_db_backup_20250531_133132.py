...
class SearxSearch:
    ...
    def _get_weather_data(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            import re
            location = "Garden Grove, CA"  # Default
            ...
            response = requests.get(weather_url, timeout=5, headers={'User-Agent': 'Yappy/1.0'})
            ...
            if response.status_code == 200:
                ...
                weather_info = f"Current weather in {city}, {region}, {country}:\n"
                ...
                weather_info += f"UV Index: {current.get('uvIndex', 'N/A')}"
                
                return [{
                    'title': f'Weather in {city}',
                    'snippet': weather_info,
                    'link': f'https://wttr.in/{quote(location)}',
                    'engine': 'wttr.in'
                }]
